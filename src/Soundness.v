...
Proof with (meta; meta_clean; eauto 2 with typ;
  ...
  - (* Local reasoning *)
    ...
    + lets: H__v2 v2 In_singleton.
      lets (? & ? & ? & ? & ?): H13 v2; [ lets: (proj1 H__st2); eauto with lia|]; meta.
...
Proof with (meta; meta_clean; eauto 2 with typ;
  ...
  + (* hot *)
    ...
    * intros l H__l.
      ...
      ++ left.
         lets (?T & ?T & ? & ? & ?): H__mn5 l...
         eexists. eexists... apply s_typ_mode...
      ++ assert (l = dom σ1 \/ l < dom σ1) as [|] by lia...
         subst. left. eexists. eexists; updates...
    * intros l H__l.
      eapply P_Hots_env...
    * exists Σ6, (dom σ1), σ4; splits; eauto with typ...
...
Lemma soundness_init_cons:
  forall n,
    (forall e ρ σ ψ r Γ Σ U T,
      ...
      (forall C flds I ρ σ Γ Σ r,
          init_soundness n C flds I ρ σ Γ Σ r) ->
    (forall f flds C I ρ σ Γ Σ r,
        init_soundness (S n) C (f::flds) I ρ σ Γ Σ r).
Proof with (meta; meta_clean; eauto 2 with typ;
            try match goal with
                | |- ?Σ ⊨ ?l : ?T => try solve [eapply vt_sub; eauto with typ]
                end).
  ...
  rewrite H12 in H8.
  lets (H__st1 & H__mn1 & H__stk1): field_initialization v0 H__st0 H8 H__field Σ1;
